...
class RecognitionModel(pl.LightningModule):
    ...
    def __init__(self, network, dataset: str, num_classes, img_shape: Tuple[int, int],
        ...
        model_input_shape = torch.tensor(img_shape + (dim, ))
        self.model = by_name(network)(dataset, model_input_shape, num_outputs=num_classes, **model_kwargs)
...
#         # accuracy = Accuracy(preds=y_prediction, target=batch.y)
#         accuracy = Accuracy(task = 'multiclass', num_classes=3).to(self.device)
#         accuracy(y_prediction, batch.y) 
